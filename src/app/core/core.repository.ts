...
export class CoreRepository {
    ...
    async retirarMonto(queryRunner, cuenta: number, valor: number): Promise<string> {
        ...
        const movimientosRepo = queryRunner.manager.getRepository(MovimientosEntity);
        // Obtener el saldo del usuario desde el proyecto de Saldos 
        ...
        try {
            ...
            if (saldo < valor) {
                const errorGlobal =  `No tiene fondos suficientes para realizar el retiro de la cuenta: ${cuenta}`;
                throw new BadRequestException(errorGlobal); //borrar esto para lo de errores viky 
                return errorGlobal;
            }
